...
fn parse_decl(it: &mut ParseIter) -> Result<Spanned<Var>, ParseErrorKind> {
  if let Some(&tok) = it.peek() {
    return match tok.node {
      Token::Sql => {
        it.next();
        let mut pieces: Vec<Spanned<Var>> = Vec::new();
        loop {
